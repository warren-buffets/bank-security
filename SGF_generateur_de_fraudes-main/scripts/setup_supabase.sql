...
-- Copiez ce contenu dans votre SQL Editor Supabase et exécutez-le

-- Supprimer les tables si elles existent (optionnel, pour réinitialiser)
-- DROP TABLE IF EXISTS synthetic_transactions CASCADE;
-- DROP TABLE IF EXISTS synthetic_batches CASCADE;

-- Table des transactions synthétiques
...
-- Table des batches (WORM - Write Once Read Many)
...
DROP TRIGGER IF EXISTS update_synthetic_transactions_updated_at ON synthetic_transactions;
CREATE TRIGGER update_synthetic_transactions_updated_at
    BEFORE UPDATE ON synthetic_transactions
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- View pour statistiques de fraude
...
BEGIN
    ...
    RAISE NOTICE 'Tables: synthetic_transactions, synthetic_batches';
    RAISE NOTICE 'Vue: fraud_statistics';
END $$;
